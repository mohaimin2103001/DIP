...
def default_results_dir() -> Path:
    ...
    candidates = [Path(name) for name in preferred]
    candidates.extend(sorted(Path(".").glob("results*")))
...
def make_branch_summary(branch_df: pd.DataFrame) -> pd.DataFrame:
    if "branch" in branch_df.columns:
        branch_df = branch_df[branch_df["branch"] == "skeleton"].copy()
...
def make_overall_ranking(method_summary: pd.DataFrame) -> pd.DataFrame:
    ...
    for split, group in method_summary.groupby("split", sort=False):
        ...
        ranked["overall_position"] = ranked["average_rank"].rank(
...
def save_metric_chart(method_summary: pd.DataFrame, charts_dir: Path) -> List[Path]:
    ...
    for split, group in method_summary.groupby("split", sort=False):
        ...
        ax.set_xticklabels(
            [
                metric.upper() if metric in {"iou", "f1"} else metric.title()
                for metric in METRICS
            ]
        )
        ax.set_ylim(0, 1.05)
...
def save_processing_time_chart(
    method_summary: pd.DataFrame, charts_dir: Path
) -> List[Path]:
    paths = []
    for split, group in method_summary.groupby("split", sort=False):
        group = group.copy()
...
def save_feature_value_charts(
    ...
    for split, group in method_summary.groupby("split", sort=False):
        ...
        fig.suptitle(
            f"{split}: predicted features vs ground truth", y=1.02, fontsize=14
        )
        fig.tight_layout()
...
def save_feature_error_heatmap(
    method_summary: pd.DataFrame, charts_dir: Path
) -> List[Path]:
    paths = []
    for split, group in method_summary.groupby("split", sort=False):
        ...
        data = np.array(
            [
                [group.loc[method, column] for column, _ in columns]
...
def save_skeleton_chart(branch_summary: pd.DataFrame, charts_dir: Path) -> List[Path]:
    paths = []
    score_cols = ["avg_dice", "avg_iou", "avg_f1"]
    for split, group in branch_summary.groupby("split", sort=False):
        group = group.copy()
        group["label"] = group["method"].astype(str)
        ...
        for idx, column in enumerate(score_cols):
            ...
            ax.bar(
                ...
                group[column],
                width=width,
                label=column.replace("avg_", "").upper(),
            )
        ax.set_xticks(x)
        ax.set_xticklabels(group["label"], rotation=35, ha="right")
        ax.set_ylim(0, 1.05)
        ax.set_ylabel("Average score")
        ax.set_title(f"{split}: skeletonization metric comparison")
        ax.grid(axis="y", alpha=0.25)
        ax.legend()
        fig.tight_layout()
        path = charts_dir / f"{split}_skeleton_metric_comparison.png"
        fig.savefig(path, dpi=160)
        plt.close(fig)
        paths.append(path)
    return paths
...
def save_all_charts(
    ...
    paths.extend(save_skeleton_chart(branch_summary, charts_dir))
...
<body>
  ...
  <main>
    <section>
      ...
      <p>Lower average rank is better. Scores use segmentation metrics, processing time, and mask feature errors.</p>
      {df_to_html(ranking)}
    </section>
    <section>
    ...
    <section>
      <h2>Skeletonization Summary</h2>
      <p>This table is built from the lean skeleton-only branch CSV generated by the refactored pipeline.</p>
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create charts, combined tables, and an HTML dashboard from the refactored FIVES pipeline CSV outputs."
    )
    parser.add_argument(
        "--results",
        type=Path,
        default=default_results_dir(),
        help="Result folder created by retinal_segmentation_pipeline.py.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    round_numeric(method_summary).to_csv(
        tables_dir / "single_easy_method_comparison_table.csv", index=False
    )
    round_numeric(branch_summary).to_csv(
        tables_dir / "skeleton_branch_summary_table.csv", index=False
    )
    round_numeric(best_methods).to_csv(
        tables_dir / "best_method_by_criterion_table.csv", index=False
    )
    round_numeric(ranking).to_csv(
        tables_dir / "overall_method_ranking_table.csv", index=False
    )
    round_numeric(image_table).to_csv(
        tables_dir / "image_level_method_comparison_table.csv", index=False
    )

    chart_paths = save_all_charts(
        method_df, method_summary, branch_summary, ranking, charts_dir
    )
    make_html_report(
        output=output,
